...
const Show = ({navigation, route}) => {
  ...
  const getJoinUserList = async hobbiesId => {
    try {
      const chatRoomsInfo = await firestore()
  ...
  const onPressToken = () => {
    ...
    if (checkMyId) {
      console.log(
        '같아서 참여하기 누르면 본인 참가 했으니 중복참가 안된다고 함',
      );
      setIsToast(true);
    } else {
    ...
    return (
      <View
        ...
        <Image
          ...
          style={{width: 60, height: 60, borderRadius: 50, marginBottom: 5}}
        />
      ...
      <View style={{flex: 1, gap: 10}}>
        ...
        <View style={{paddingHorizontal: 16, paddingTop: 10, gap: 8}}>
          ...
          <View
            ...
            <Text>
              {dayjs(deadline).format('YYYY년 MM월 DD일 (ddd) HH:mm')} 까지
            </Text>
          </View>
          <View
            style={{
